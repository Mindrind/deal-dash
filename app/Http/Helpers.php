...
if (!function_exists('home_price')) {
    function home_price($product, $formatted = true)
    {
        if($product->auction_product == 1){
            $lowest_price = $product->starting_bid;
            $highest_price = $product->starting_bid;
        }else{
...
if (!function_exists('getUserName')) {
    ...
    function getUserName($user_id)
    {
        if(!empty($user_id)){
            ...
            return $user['name']; // $user = null means we did not get any match with the email provided by the user inside the database
            }
            else {
                return '';
            }
