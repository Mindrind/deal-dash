...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $auction_products = Product::orderBy('created_at', 'desc')->where('auction_product',1)->where('auction_winned',0);
        $auction_products = $auction_products->paginate(15);
        
         $winned_auction_products = Product::orderBy('created_at', 'desc')->where('auction_product',1)->where('auction_winned',1);
        $winned_auction_products = $winned_auction_products->paginate(15);

        $auction_won_products = Product::where("auction_product", 1)->where('auction_winned',0)->get();

        return view('frontend.index', 
        compact('featured_categories', 'todays_deal_products', 'newest_products','auction_products', 'auction_won_products','winned_auction_products'));
    }
    ...
    public function product(Request $request, $slug)
    {
        ...
        // remove bcoz not oppening auction product with slug ->where('auction_product', 0)
