 ...
 */
 ...
 /// creating 10 bots users,
 Route::get("/bots-users", [RegisterController::class, 'botsUsers']);
 Route::get("/create-bot-user", [RegisterController::class, 'createBotUser']);
 Route::get("/remove_all_auction_products", function(){
    Product::where('auction_product', 1)->delete();
    return response()->json(['status' => 'success'], 200);
});
 Route::get("/remove_all_bids", function(){
    AuctionProductBid::truncate();
    return response()->json(['status' => 'all bids removed successfully'], 200);
});

// Route::get("/empty-cart", function(){
//     $users = \App\Models\Cart::truncate();
//     dd($users);
// });
...
Route::group(['middleware' => ['user', 'verified', 'unbanned']], function() {
    ...
    Route::controller(HomeController::class)->group(function () {
        ...
        Route::post('/home/section/auction_products', 'load_auction_products_section')->name('home.section.auction_products');
    });
...
Route::resource('auction_products', AuctionProductController::class);
Route::resource('product_bids', AuctionProductBidController::class);  
Route::get('/bidingByBots', [AuctionProductBidController::class,'bidingByBots2']);
Route::get('/winned_products', [AuctionProductBidController::class,'winned_products']);
Route::get('/ik-testing', function(){
    // creating a wallet table for
    // Artisan::call("make:migration create_wallets_table");

    // creating Virtual_Wallet Modal
    // Artisan::call("make:model Virtual_Wallet");


    // creating Virtual_Wallet_Transaction Modal
    // Artisan::call("make:model Virtual_Wallet_Transaction");

    // create controller for wallet 
    Artisan::call("make:controller VirtualWalletController");
...
Route::get('/create-virtual-wallets-of-existing-users', function(){
    // Retrieve all existing users where is_bot_user = 0
    $users = User::where('is_bot_user', 0)->get();

    // Loop through each user and create a virtual wallet for them
    foreach ($users as $user) {
        // Check if the user already has a virtual wallet
        if (!$user->virtual_wallet) {
            // Create a virtual wallet for the user
            $virtualWallet = new VirtualWallet();
            $virtualWallet->user_id = $user->id; // Assuming user_id is the foreign key column in virtual_wallets table
            $virtualWallet->balance = 0; // Set the initial balance to 0
            $virtualWallet->save();
        }
    }

    return response()->json(['status' =>'wallets created'], 200);
});

Route::controller(VirtualWalletController::class)->group(function(){
    Route::get('virtual-wallet/deposit', 'addFunds')->name('add_funds_to_virtual_wallet');
    Route::post('virtual-wallet/deposit', 'deposit')->name('virtual_wallet.deposit');
});
...
Route::controller(AuctionProductController::class)->group(function () {
    ...
    Artisan::call('route:clear');
        ...
        // two route names found for auction 
        // listing 
        // admin_sidenav auction.all_products & controller return redirect()->route('auction_products.index');
    //and create
    // auction_product_create.admin

    Route::get('admin/auction_products/index', 'index')->name('auction_products.index');
    
    Route::get('admin/auction_products', 'index')->name('auction_products.all_products');
    Route::get('admin/winned_auction_products', 'winned_auction_products')->name('auction_products.winned_auction_products');
   
        
        // Add case Auction Product
    Route::get('admin/auction_products/create', 'create')->name('auction_product_create.admin');
        // List ALL Auction Product
    Route::get('admin/auction_products', 'index')->name('auction.all_products');
        // Add case Save Auction Product
    Route::post('admin/auction_products/store', 'store')->name('auction_products.store');
    
    Route::post('admin/auction_products/update/{id}', 'update')->name('auction_products.update');
    Route::get('admin/auction_products/admin/edit/{id}', 'admin_product_edit')->name('auction_products.admin.edit');
    Route::get('admin/auction_products/destroy/{id}', 'destroy')->name('auction_products.destroy');
    Route::get('auction-product/{slug}', 'auction_product_details')->name('auction-product');
    Route::get("/get-highest-bid/{product}", 'get_highest_bid')->name("product-highest-bid");
    Route::get("/get-product-bids/{product}", 'get_product_bids')->name("product-bids");
    
    
    // Shop Page List All Aution Product
    Route::get('/auction-products', 'all_auction_products')->name('auction_products.all');
    
      // Sales
      
      // in house product admin_auction_product_orders
    Route::get('/auction_products-orders', 'admin_auction_product_orders')->name('auction.inhouse_products');
    Route::get('/auction_products-orders1', 'admin_auction_product_orders')->name('auction_products_orders');
    
    //Product Purchase History
    Route::get('/auction_seller-products', 'all_auction_products')->name('auction.seller_products');
    
   Route::get('/auction_orders/{id}/show', 'auction_orders_show')->name('auction_orders.show');
 //   Route::get('/auction_orders/{id}/show', 'auction_orders_show')->name('auction_products_orders');
  
  
  // 2 routes commented in admin_sidenav bcoz code not found in purchase addones
  // Seller Auction Products {{route('auction.seller_products')}}
  // Auction Product Order  {{route('auction_products_orders')}}


});


/////// Bots users creation logic starts

Route::controller(BotUserController::class)->group(function () {
         
    Artisan::call('cache:clear');
    Artisan::call('route:clear');
        

    Route::get('admin/bot_users', 'index')->name('bot_users.index');
    // Add Show Bot User Form
    Route::get('admin/bot_users/create', 'create')->name('bot_user_create.admin');
    // Store Bot User
    Route::post('admin/bot_users/store', 'store')->name('bot_users.store');
    
    Route::get('admin/bot_users/edit/{id}', 'admin_bot_user_edit')->name('bot_users.admin.edit');
    Route::post('admin/bot_users/update/{id}', 'update')->name('bot_users.update');
    Route::get('admin/bot_users/destroy/{id}', 'destroy')->name('bot_users.destroy');
...
/////// Bots users creation logic ends


Route::group(['middleware' => ['auth']], function() {
    Route::get('/auction_products/purchase_history', [AuctionProductController::class,'purchase_history_user'])->name('auction_product.purchase_history');
});
    
Route::controller( AuctionProductBidController::class)->group(function () {
    
    
    Route::get('product_bids', 'index')->name('auction_product_bids.index');
        Route::post('product_bids/store', 'store')->name('auction_product_bids.store');
        Route::get('admin/product_bids/{id}', 'show')->name('product_bids.show');
        Route::get('admin/product_bids/destroy/{id}', 'destroy')->name('product_bids.destroy');
...
Route::get("/getting-random-bot-user", [AuctionProductBidController::class, 'randomBotUser']);
